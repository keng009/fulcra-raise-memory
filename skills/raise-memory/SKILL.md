...
<!-- Trigger phrases: snapshot — "show me my last 30 days", "snapshot my raise";
     ...
     sourcing — "talked to this fund before?", a pasted intro
...
Trigger: "have I talked to Meridian before?", "talked to this fund before?", "what do I know about this?" with a pasted intro email or deck blurb — the moment a new opportunity arrives.

1. **Extract entities** from whatever was pasted or named: fund name, partner names. Zero questions unless extraction genuinely fails.
...
4. **Staleness at scale.** Computing "going cold" never requires reading every relationship file: one windowed `get_records` call gives the active set; going-quiet = INDEX entries minus that set (ADR-0006 access rules).
