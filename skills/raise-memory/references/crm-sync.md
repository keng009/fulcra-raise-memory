...
5. **Dedupe by key before every write.** Every touchpoint has a deterministic key in one of the canonical forms defined in `conventions.md` — date-form `touch:<person-slug>:<YYYY-MM-DD>` (with `-2`/`-3` ordinals), calendar `touch:cal:<event-id>`, transcript `touch:<transcript-id>`, CRM-note `touch:<crm>-note:<note-id>`, or messaging-thread `touch:<tool>-thread:<id>`. Before writing a note, fetch the matched contact's existing notes and scan their titles for the key string. If any title contains it, skip the write and tell the user that touchpoint is already in the CRM. Never assume a write happens exactly once. **Cross-system dedupe (coexistence):** the same scan also checks titles and body previews for the touchpoint's *source id* in any format — the transcript id, calendar event id, or mail-thread id, whether it appears as `[touch:<id>]`, `[otter:<id>]`, in a `Source:` line, or inside a URL. Any hit means another logger (a sibling packet, a private sync, a CRM auto-logger, a human) already recorded this conversation in the CRM: skip the write, count it as skipped-duplicate, name the note, and never delete or rewrite it. This skill never depends on another logger existing; it only refuses to write a second note for the same source.
...
## HubSpot (Tier W — official connector is write-capable; untested under this flavor)

Claude's official HubSpot connector is **write-capable** as of September 2026 (`manage_crm_objects`; earlier versions were read-only — detect by capability, never by the connector's name or age). The note write was live-tested in the sales sibling (design evidence only, ADR-0007); under this flavor say "untested" on first use and read the first note back.
