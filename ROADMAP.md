...
This packet ships engine-proven, flavor-untested ([testing.md](docs/testing.md)). The release gate is [#1](https://github.com/keng009/fulcra-raise-memory/issues/1): a full `raise-demo` session through Claude's real zip-upload flow, and a `raise-memory` snapshot → commit → veto run on a real account. Plus a fresh-eyes review pass of the founder vocabulary and claims ([#2](https://github.com/keng009/fulcra-raise-memory/issues/2)) and CI workflows landing ([#4](https://github.com/keng009/fulcra-raise-memory/issues/4)).
...
[fulcra-dealflow-memory](https://github.com/keng009/fulcra-dealflow-memory) serves investors managing deal flow — the origin of this engine, with the live-test evidence and [its own roadmap](https://github.com/keng009/fulcra-dealflow-memory/blob/main/ROADMAP.md). Features diverge by ICP; engine fixes cherry-pick.
